...
const Profile = ({ data }, context) => {
  ...
  const handleSubmit = () => {
    ...
    backendApi.patch(`users/${userId}`, data, {
      ...
      <div className="xs:container sm:container md:container lg:container xl:container mx-auto flex flex-col mt-20 lg:mt-28">
        ...
        <div className="flex flex-col items-center mt-8 lg:mt-11">
          <div className="relative w-36 h-36 lg:w-52 lg:h-52">
            {profile.avatarPreview ? (
              <img src={profile.avatarPreview} alt="camera" className="w-full h-full rounded-full" />
            ) : (
              <Image src={profile.avatar !== null ? `http://localhost:4000/files/${profile.avatar}` : '/people.png'} alt="people" width="215px" height="215px" className="rounded-full" />
            )}
            ...
            <input type="file" name="avatar" id="avatar" className="hidden" onChange={handleInputFile} />
          </div>
          <h5 className="fontPlayfair text-3xl lg:text-5xl font-bold mt-4 lg:mt-6 text-center">{profile.name}</h5>
          <p className="font-bold text-lg lg:text-2xl text-gray-400 mt-4 lg:mt-6">{profile.email}</p>
          <p className="font-bold text-lg lg:text-2xl text-gray-400 mt-0 lg:mt-1">{profile.phone !== null ? profile.phone : ''}</p>
          <p className="font-bold text-lg lg:text-2xl text-gray-400 mt-0 lg:mt-1">Has been active since {profile.createdAt.slice(0, 4)}</p>
        </div>
        <div className="flex justify-center mt-5 lg:mt-10">
          <div className="mx-10 lg:mx-20 flex items-center">
            <input type="radio" id="male" name="gender" value="male" checked={profile.gender === 'male'} className="w-4 h-4 lg:w-7 lg:h-7" onChange={handleInput} />
            <label htmlFor="male" className="ml-2 lgml-5 text-lg lg:text-2xl">Male</label>
          </div>
          <div className="mx-10 lg:mx-20 flex items-center">
            <input type="radio" id="female" name="gender" value="female" checked={profile.gender === 'female'} className="w-4 h-4 lg:w-7 lg:h-7" onChange={handleInput} />
        ...
        <div className="flex flex-col mt-12 lg:mt-20">
          <div className="flex flex-col">
            ...
            <input id="email" type="text" name="email" placeholder="Email address" className="py-2 lg:py-4 border-b focus:outline-none bg-white border-black text-black text-lg lg:text-2xl" value={profile.email} onChange={handleInput} />
          </div>
          <div className="flex flex-col mt-4">
            <label htmlFor="address" className="text-gray-400 text-lg lg:text-2xl">Address :</label>
            <input id="address" type="text" name="address" placeholder="Address" className="py-2 lg:py-4 border-b focus:outline-none bg-white border-black text-black text-lg lg:text-2xl" value={profile.address} onChange={handleInput} />
          </div>
          <div className="flex flex-col mt-4">
            <label htmlFor="phone" className="text-gray-400 text-lg lg:text-2xl">Mobile number :</label>
            <input id="phone" type="text" name="phone" placeholder="Mobile number" className="py-2 lg:py-4 border-b focus:outline-none bg-white border-black text-black text-lg lg:text-2xl" value={profile.phone !== null ? profile.phone : ''} onChange={handleInput} />
          </div>
        </div>
        ...
        <div className="flex flex-col mt-9 lg:mt-16">
          ...
          <di className="flex justify-between">
            <div className="flex flex-col mt-4 w-1/2 pr-4 lg:pr-9">
              ...
              <input id="name" type="text" name="name" placeholder="Display name" className="py-2 lg:py-4 border-b focus:outline-none bg-white border-black text-black text-lg lg:text-2xl" value={profile.name} onChange={handleInput} />
            </div>
            <div className="flex flex-col mt-4 w-1/2 pl-4 lg:pl-9">
              <label htmlFor="birth" className="text-gray-400 text-lg lg:text-2xl">MM/DD/YYYY</label>
              <input id="birth" type="date" name="birth" placeholder="Birth date" className="py-2 lg:py-4 border-b focus:outline-none bg-white border-black text-black text-lg lg:text-2xl" value={profile.birth !== null ? profile.birth : ''} onChange={handleInput} />
            </div>
          </di>
        </div>

        <div className="flex justify-between my-12 lg:my-24">
          <button type="button" className="bg-yellow-300 hover:bg-yellow-400 py-3 lg:py-6 rounded-lg font-black text-sm lg:text-2xl w-28 lg:w-80" onClick={handleSubmit}>Save Change</button>
...
export const getServerSideProps = requireAuthentication(
  async(ctx) => {
    ...
    const user = await fetch(`${process.env.NEXT_BACKEND_API}v1/users/${userId}`, {
      withCredentials: true,
      headers: {
        Cookie: 'token=' + token
      }
